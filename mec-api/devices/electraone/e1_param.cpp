#include "e1_param_2.h"

//#include <iostream>

namespace mec {

bool ElectraOneParamMode::init() {
    selectedParamIdx_ = 0;
    return ElectraOneParamMode1::init();
}

void ElectraOneParamMode::onEncoder(unsigned id, int value) {
    if (buttonState_[2]) {
        // change module/page functionality
        ElectraOneParamMode1::onEncoder(id, value);
    } else {
        switch (id) {
            case 0: {
                break;
...
void ElectraOneParamMode::setCurrentPage(unsigned pageIdx, bool UI) {
    ...
    return ElectraOneParamMode1::setCurrentPage(pageIdx, UI);
}
